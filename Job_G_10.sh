...
#SBATCH --job-name=G_10dB
#SBATCH --output=G_10dB_job.log
#SBATCH --error=G_10dB_job.err
...
(while true; do nvidia-smi >> gpu_monitor_G_10dB.log 2>&1; sleep 60; done) &
...
# ==============================================
# Gaussian 10dB Noise Experiment (15 epochs)
# ==============================================
...
apptainer exec --nv \
    --env LD_LIBRARY_PATH=/apps/Common/software/CUDA/11.8.0/lib64:\$LD_LIBRARY_PATH \
    $CONTAINER \
    aucurriculum train -cn Gaussian_10dB_curriculum device=cuda ++dataset.path=$DATA_PATH
...
apptainer exec --nv \
    --env LD_LIBRARY_PATH=/apps/Common/software/CUDA/11.8.0/lib64:\$LD_LIBRARY_PATH \
    $CONTAINER \
    aucurriculum train -cn Gaussian_10dB_anticurriculum device=cuda ++dataset.path=$DATA_PATH

...
#SBATCH --job-name=G_5dB
#SBATCH --output=G_5dB_job.log
#SBATCH --error=G_5dB_job.err
...
(while true; do nvidia-smi >> gpu_monitor_G_5dB.log 2>&1; sleep 60; done) &
...
apptainer exec --nv \
    ...
    $CONTAINER \
    aucurriculum train -cn Gaussian_5dB device=cuda ++dataset.path=$DATA_PATH
...
apptainer exec --nv \
    --env LD_LIBRARY_PATH=/apps/Common/software/CUDA/11.8.0/lib64:\$LD_LIBRARY_PATH \
    $CONTAINER \
    aucurriculum curriculum -cn Gaussian_5dB_scoring device=cuda ++dataset.path=$DATA_PATH
...
apptainer exec --nv \
    --env LD_LIBRARY_PATH=/apps/Common/software/CUDA/11.8.0/lib64:\$LD_LIBRARY_PATH \
    $CONTAINER \
    aucurriculum train -cn Gaussian_5dB_curriculum device=cuda ++dataset.path=$DATA_PATH
...
apptainer exec --nv \
    --env LD_LIBRARY_PATH=/apps/Common/software/CUDA/11.8.0/lib64:\$LD_LIBRARY_PATH \
    $CONTAINER \
    aucurriculum train -cn Gaussian_5dB_anticurriculum device=cuda ++dataset.path=$DATA_PATH

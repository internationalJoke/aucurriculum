...
#SBATCH --job-name=G_0dB
#SBATCH --output=G_0dB_job.log
#SBATCH --error=G_0dB_job.err
...
(while true; do nvidia-smi >> gpu_monitor_G_0dB.log 2>&1; sleep 60; done) &
...
# ==============================================
# Gaussian 0dB Noise Experiment (15 epochs)
# ==============================================

# Step 1: Standard training with 0dB noise (generates checkpoints for scoring)
echo "=== Step 1: Standard Training with Gaussian 0dB ==="
apptainer exec --nv \
    --env LD_LIBRARY_PATH=/apps/Common/software/CUDA/11.8.0/lib64:\$LD_LIBRARY_PATH \
    $CONTAINER \
    aucurriculum train -cn Gaussian_0dB device=cuda ++dataset.path=$DATA_PATH
...
apptainer exec --nv \
    --env LD_LIBRARY_PATH=/apps/Common/software/CUDA/11.8.0/lib64:\$LD_LIBRARY_PATH \
    $CONTAINER \
    aucurriculum curriculum -cn Gaussian_0dB_scoring device=cuda ++dataset.path=$DATA_PATH
...
apptainer exec --nv \
    --env LD_LIBRARY_PATH=/apps/Common/software/CUDA/11.8.0/lib64:\$LD_LIBRARY_PATH \
    $CONTAINER \
    aucurriculum train -cn Gaussian_0dB_curriculum device=cuda ++dataset.path=$DATA_PATH
...
apptainer exec --nv \
    --env LD_LIBRARY_PATH=/apps/Common/software/CUDA/11.8.0/lib64:\$LD_LIBRARY_PATH \
    $CONTAINER \
    aucurriculum train -cn Gaussian_0dB_anticurriculum device=cuda ++dataset.path=$DATA_PATH
